        ...
        RageUI.Visible(RMenu:Get("ambulance", "main"), true)
            CreateThread(function()
				...
				RageUI.IsVisible(RMenu:Get('ambulance', 'main'), true, true, true, function()
					RageUI.Checkbox("Prendre/Quitter son service", nil, ambuservice, { Style = RageUI.CheckboxStyle.Tick }, function(Hovered, Selected, Active, Checked)
						...
						if ambuservice then
							...
							RageUI.Button("Intéraction citoyens", nil, {RightLabel = "~b~Intéragir ~s~→→"}, true, function(Hovered, Active, Selected)
							end, RMenu:Get('ambulance', 'citoyens'))

											
							RageUI.Button("Intéraction Appels", nil, {RightLabel = "~b~Intéragir ~s~→→"}, true, function(Hovered, Active, Selected)
							...
							RageUI.Button("Donner une facture",nil, {RightLabel = "~b~Facturer~s~ →→"}, true, function(_,_,s)
							...
							RageUI.Button("[~y~Annonce~s~] :~g~ Disponible", nil, {RightLabel = "→→"}, true, function(Hovered, Active, Selected)
								if (Selected) then
									TriggerServerEvent('EMS:Ouvert')
								end
							end)
				
							RageUI.Button("[~y~Annonce~s~] :~r~ Non disponible", nil, {RightLabel = "→→"}, true, function(Hovered, Active, Selected)
								if (Selected) then
				...
				RageUI.IsVisible(RMenu:Get('ambulance', 'citoyens'), true, true, true, function()
					...
					RageUI.Button("Réanimer la Personne", nil, { RightBadge = RageUI.BadgeStyle.Heart },true, function(Hovered, Active, Selected)
						if Selected then 
							revivePlayer(closestPlayer)    
						end
					end)
		
					RageUI.Button("Soigner une petite blessure", nil, { RightBadge = RageUI.BadgeStyle.Heart },true, function(Hovered, Active, Selected)
						if (Selected) then 
					...
					RageUI.Button("Soigner une plus grande blessure", nil, { RightBadge = RageUI.BadgeStyle.Heart },true, function(Hovered, Active, Selected)
						if (Selected) then 
							local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
							if closestPlayer == -1 or closestDistance > 1.0 then
								ESX.ShowNotification('Aucune Personne à Proximité')
				...
				RageUI.IsVisible(RMenu:Get('ambulance', 'lister'), true, true, true, function()
					...
					RageUI.Button("Effacer les appels en cours", nil, {RightLabel = "→→"}, true, function(Hovered, Active, Selected)
						if Selected then
					...
					for numreport = 1, #reportlistesql, 1 do
					  RageUI.Button("[~y~Patient ~s~: "..reportlistesql[numreport].reporteur.."~s~] - ~b~Numéro ~s~: "..reportlistesql[numreport].id, nil, { RightLabel = "~r~En ATTENTE"}, true, function(Hovered, Active, Selected)
						  if (Selected) then
...
local Garage = false
RMenu.Add('Voiture', 'main', RageUI.CreateMenu("Garage", "", 10,80))
					...
					RageUI.IsVisible(RMenu:Get('Voiture', 'main'), true, true, true, function()
						...
						for index,infos in pairs(Cloak.vehicles.car) do
							if infos.category ~= nil then 
								...
								RageUI.Button(infos.label,nil, {RightBadge = RageUI.BadgeStyle.Car}, ESX.PlayerData.job.grade >= infos.minimum_grade, function(_,_,s)
									if s then
										Citizen.CreateThread(function()
...
-- Garage Hélicoptère 
local Helico = false
RMenu.Add('Helico', 'main', RageUI.CreateMenu("Helicoptère", "", 10,80))
...
RMenu.Add('cloakroom', 'main', RageUI.CreateMenu("Vestiaires", "", 0,0))
				...
				RageUI.IsVisible(RMenu:Get('cloakroom', 'main'), true, true, true, function()
					...
					RageUI.Separator("↓ ~b~Tenues spéciales ~s~↓")
						...
						for index,infos in pairs(Cloak.clothes.specials) do
							RageUI.Button(infos.label,"Vous permets d'équiper: "..infos.label, {RightBadge = RageUI.BadgeStyle.Clothes}, ESX.PlayerData.job.grade >= infos.minimum_grade, function(_,_,s)
								if s then
									ExecuteCommand("e me")
									Citizen.Wait(1400)
...
local Pharma = false
RMenu.Add('pharma', 'main', RageUI.CreateMenu("Pharmacie", "", 0,0))
				...
				RageUI.IsVisible(RMenu:Get('pharma', 'main'), true, true, true, function()
					RageUI.Separator("↓ ~o~Blessure Graves ~s~↓")
						for index,infos in pairs(Config.Hospitals.CentralLosSantos.Pharmacies) do
							RageUI.Button("Medikit ~b~(x1)", nil, {RightBadge = RageUI.BadgeStyle.Heart}, true, function(_,_,s)
								if s then
						...
						for index,infos in pairs(Config.Hospitals.CentralLosSantos.Pharmacies) do
							RageUI.Button("Bandages ~b~(x1)", nil, {RightBadge = RageUI.BadgeStyle.Heart}, true, function(_,_,s)
								if s then
